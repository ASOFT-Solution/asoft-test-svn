...
--- Modify by Tấn Đạt, Date 23/02/2018: Load thêm dữ liệu Hợp đồng(CRMT20001)
-- <Example>
/* 
...
Select C.APK, C.DivisionID
, C.ContractID as SOrderID, C.VoucherTypeID, C.ContractNO as VoucherNo
, C.ContractDate as OrderDate, C.ObjectID, C.ObjectName
, C.DeliveryAddress, C.Notes, C.Disabled, C.OrderStatus, AT0099.Description as OrderStatusName
, C.CreateDate, C.CreateUserID, C.LastModifyUserID, C.LastModifyDate, C.TranMonth, C.TranYear, A03.FullName EmployeeID
, C.SalesManID, C.ShipDate,  C.IsConfirm, AT01.Description as IsConfirmName, C.DescriptionConfirm
, C.ConfirmDate, C.ConfirmUserID
, C.IsInvoice, Sum(A.ConvertedAmount) as TotalAmount
FROM CRMT20001 C With (NOLOCK)
Inner join CRMT20002 A  With (NOLOCK) ON A.DivisionID = C.DivisionID and A.ContractID =C.ContractID
Left join AT0099 With (NOLOCK) on Convert(varchar, C.OrderStatus) = AT0099.ID and AT0099.CodeMaster = ''AT00000003''
Left join AT1103 A03 With (NOLOCK) on C.EmployeeID = A03.EmployeeID
Left join AT0099 AT01 With (NOLOCK) on Convert(varchar, C.IsConfirm) = AT01.ID and AT01.CodeMaster = ''AT00000039''
--WHERE '+@sWhere+'
Group by C.APK, C.DivisionID, C.ContractID, C.VoucherTypeID, C.ContractNO, C.ContractDate
	, C.ObjectID, C.ObjectName , C.DeliveryAddress, C.Notes, C.Disabled, C.OrderStatus, AT0099.Description
	, C.CreateDate, C.CreateUserID, C.LastModifyUserID, C.LastModifyDate, C.TranMonth, C.TranYear, A03.FullName 
	, C.SalesManID, C.ShipDate,  C.IsConfirm, AT01.Description, C.DescriptionConfirm
	, C.ConfirmDate, C.ConfirmUserID, C.IsInvoice
	--, C.SOrderType
	Declare @Count int
