IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FindGUIDTable]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[FindGUIDTable]
GO

CREATE PROCEDURE [FindGUIDTable] @searchguid uniqueidentifier
AS
...
--set  @searchguid = ltrim(rtrim('c91a9e73-da98-48f1-9560-fcb7b110ac4a'))
...
begin
 ...
 where ttbs.[RowId] between @lowrow + 1 and @lowrow + 15
  and ttbs.[IsGuidType] is not null
  and (IsGuidType=0 or @SrchCharTypes=1)
...
select distinct ttbs.[TableName],
 ttbs.[ColName],
 f.[RowsFound],
 'select * ' + ttbs.SQLQuery as sqlQuery
from #Found f
