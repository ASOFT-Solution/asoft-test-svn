...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[MP7229]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[MP7229]
GO
...
----- purpose: Phan bo CP nhan cong cho TP theo PP dinh muc  & SPDD khong phan bo chi phi

/**********************************************
** Edited by: [GS] [Cẩm Loan] [03/08/2010]
***********************************************/

CREATE PROCEDURE  [dbo].[MP7229] 	@DivisionID as nvarchar(50), 
...
Set 	@SumProductConverted = (Select  Sum(Isnull(ConvertedUnit,0)*Isnull(MT4444.ProductQuantity,0))
				From MT1603 inner join MT4444  on MT4444.ProductID = MT1603.ProductID and MT4444.ResultTypeID = 'R01'
...
Set @ProductlD_cur  = CURSOR  SCROLL  KEYSET  FOR 
		...
		From MT4444 inner join MT1603 on MT4444.ProductID = MT1603.ProductID and MT4444.ResultTypeID = 'R01'
...
BEGIN
	IF @ConvertedAmount >0  and  Isnull(@SumProductConverted,0)<>0
	BEGIN			
