IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[HP0263]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[HP0263]
GO
...
--- Purpose: Store tra ra du lieu truy van thong ke san xuat

CREATE PROCEDURE [dbo].[HP0263] 
...
set @sSQL = N'SELECT * FROM (
	...
	Where HT0260.DivisionID = ''' + @DivisionID + '''
		...
		And WorkingDate between ''' + convert(varchar(20), @FromDate,101) + ''' And ''' + convert(varchar(20), @ToDate,101) + '''
