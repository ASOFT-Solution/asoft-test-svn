IF EXISTS (SELECT TOP 1 1 FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[DBO].[AP8141]') AND OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [DBO].[AP8141]
GO
...
-- <Example>
/*
 AP8141 @DivisionID = 'VG', @UserID = 'ASOFTADMIN', @ImportTemplateID = '', @XML = ''
 */
 
CREATE PROCEDURE AP8141
( 
...
(
	...
	SET @SQL =  STUFF((		SELECT'ALTER TABLE #Data ADD ' + TLD.ColID + ' ' +  BTL.ColSQLDataType + 
							...
							WHERE	TL.ImportTemplateID = 'ProduceAssign'				
							FOR XML PATH('')),1,0,'')
EXEC(@SQL)


-- Thêm dữ liệu vào bảng tạm
INSERT INTO #Data ([Row],DivisionID,Period,TimesID,EmployeeID,DepartmentID,TeamID,ProductID,Quantity,TrackingDate)
SELECT	X.Data.query('Row').value('.', 'INT') AS Row,
		...
		(CASE WHEN X.Data.query('TeamID').value('.', 'NVARCHAR(50)') = '' 
					THEN '' 
					ELSE X.Data.query('TeamID').value('.', 'NVARCHAR(50)') END) AS TeamID,
...
SET @SQL1 = STUFF((	SELECT DISTINCT 
					'EXEC HP2414 @TimesID=N'''+DT.TimesID+''',@DivisionID=N'''+DT.DivisionID+''',@DepartmentID=N'''+DT.DepartmentID+''',@TeamID=N''' +CASE WHEN ISNULL(DT.TeamID,'''') = ''''THEN '%' ELSE  DT.TeamID END +''',@EmployeeID=N'''+DT.EmployeeID+''',@TranMonth='+ @TranMonth +', @TranYear='+@TranYear +',@CreateUserID='''+@UserID+''',@ProductID='''+DT.ProductID+''',@Quantity='+CAST(DT.Quantity AS VARCHAR)+',@TrackingDate='''+ CONVERT(VARCHAR(10),DT.TrackingDate,101)+'''  '								 
					FROM #Data DT 
					FOR XML PATH('')),1,0,'')
EXEC (@SQL1)
-----------------------------------------------------------------------------------------
