...
---- Modified on ... by ...
---- <Example>
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[ET9002]') AND type in (N'U'))
CREATE TABLE [dbo].[ET9002](
	[APK] [uniqueidentifier] DEFAULT NEWID(),
	[DivisionID] [nvarchar](3) NOT NULL,
	[TemplateTransactionID] [nvarchar](50) NULL,
	[TemplateID] [nvarchar](50) NULL,
	[Description] [nvarchar](250) NULL,
	[IsStockVoucher] [tinyint] NULL,
	...
	CONSTRAINT [PK_ET9002] PRIMARY KEY NONCLUSTERED 
	(
		[APK] ASC
	)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
) ON [PRIMARY]
---- Add Columns
If Exists (Select * From sysobjects Where name = 'ET9002' and xtype ='U') 
Begin 
If not exists (select * from syscolumns col inner join sysobjects tab 
On col.id = tab.id where tab.name =   'ET9002'  and col.name = 'Ana06ID')
Alter Table  ET9002 Add Ana06ID nvarchar(50) Null,
