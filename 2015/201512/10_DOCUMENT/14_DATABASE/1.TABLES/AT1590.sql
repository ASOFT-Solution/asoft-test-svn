...
---- Modified on ... by ...
---- <Example>
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[AT1590]') AND type in (N'U'))
CREATE TABLE [dbo].[AT1590](
 ...
 CONSTRAINT [PK_AT1590] PRIMARY KEY CLUSTERED 
(
	[TransactionID] ASC,
	[TableID] ASC
...
---- Add Columns
If Exists (Select * From sysobjects Where name = 'AT1590' and xtype ='U') 
Begin 
If not exists (select * from syscolumns col inner join sysobjects tab 
On col.id = tab.id where tab.name =   'AT1590'  and col.name = 'Ana06ID')
Alter Table  AT1590 Add Ana06ID nvarchar(50) Null,
