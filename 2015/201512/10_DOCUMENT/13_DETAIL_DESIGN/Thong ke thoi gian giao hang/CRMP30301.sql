...
--Check Para DivisionIDList null then get DivisionID 
 ...
 SET @sSQL = N'
	...
	(Select x.Division, x.DivisionName, x.TDate,x.TMonth, x.TYear,
		...
		where '+@sWhere+' and
		 B.KindVoucherID =3 and B.IsWeb=1
		--Xác định thời gian giao hàng của nhân viên
		)x
		Group by x.TDate, x.TMonth, x.TYear, x.Division, x.DivisionName
		)y Inner join 
	(SELECT  B.DivisionID,Day(B.VoucherDate) as TDate, Month(B.VoucherDate) as TMonth, Year(B.VoucherDate) as TYear, Count(B.VoucherID) as SumNo FROM AT2006 B
      WHERE '+@sWhere1+'
     AND B.IsWeb=1 AND B.KindVoucherID=3
	 Group by B.VoucherDate, B.DivisionID
