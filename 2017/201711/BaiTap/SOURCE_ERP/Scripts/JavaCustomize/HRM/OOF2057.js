...
$(document).ready(function () {
    ...
    GridOOT2070 = $("#GridEditOOT2070").data("kendoGrid");
    GridOOT2070.hideColumn("APK");
    if ($("#IsLock").val() == "1") {
        $("#BtnSave").kendoButton({
            enable: false
        });
    ...
    GridOOT2070.unbind("dataBound");
    var columns = GridOOT2070.columns;
    var name = 'GridEditOOT2070';
    $(GridOOT2070.tbody).off("keydown mouseleave", "td").on("keydown mouseleave", "td", function (e) {
        ...
        var isDefaultLR = $(GridOOT2070.element).attr('isDefaultLR');
        if (editor != undefined) {
            var elm = $(this);
            if (e.shiftKey) {
                switch (e.keyCode) {
...
function StatusChange() {
    GridOOT2070 = $('#GridEditOOT2070').data('kendoGrid');
    var data = GridOOT2070.dataSource.data();
    var totalNumber = data.length;

    if ($("#StatusOOT9001").val() == 1) {
        var input = $("#GridEditOOT2070").find("input#CbGridEdit_Status[type='checkbox']");
        $(input).each(function () {
...
function CustomerCheck() {
    if ($("#StatusOOT9001").val() == 0) {
        ...
        ASOFT.form.displayError("#OOF2057", msg);
        return true;
    }
    return false;
}
