...
$(document).ready(function () {
    OOF2020.grid = $('#GridOOT9000').data('kendoGrid');

    var cboDepartment = $("#DepartmentID_OOF2020").data("kendoComboBox");
    cboDepartment.bind("change", function (e) {
        $("#SectionID_OOF2020").data("kendoComboBox").text('');
        $("#SubsectionID_OOF2020").data("kendoComboBox").text('');
        $("#ProcessID_OOF2020").data("kendoComboBox").text('');

        cboDepartment.input.focus();
    });

    var cboSection = $("#SectionID_OOF2020").data("kendoComboBox");
    ...
    var cboSubsection = $("#SubsectionID_OOF2020").data("kendoComboBox");
...
function PrintClick() {
    ...
    OOF2020.doPrintOrExport(true);
   
}

function onAfterFilter() {
    OOF2020.isSearch = true;
    var datamaster = ASOFT.helper.dataFormToJSON("FormFilter");
    sessionStorage.setItem('dataFormFilter_OOF2020', JSON.stringify(datamaster));
}

function onAfterClearFilter() {
    OOF2020.isSearch = false;
...
function onRefreshGrid() {
    var totalPage = OOF2020.grid.pager.options.buttonCount;
    var CurPage = OOF2020.grid.pager.page();
    if (IsCheckExecute) {
        OOF2020.grid.dataSource.page(CurPage <= totalPage ? CurPage : 1);
        IsCheckExecute = false;
    } else {
        OOF2020.grid.dataSource.page(1);
    }
}
...
OOF2020 = new function () {
    ...
    this.doPrintOrExport = function (ischecked) {
        ...
        var dataFormFilter = sessionStorage.getItem('dataFormFilter_OOF2020');
        ...
        data.IsSearch = OOF2020.isSearch;
        ...
        data.ReportID = 'OOF2082Report';
        isPrint = ischecked;

        data.APKList = args.join(",");
        var urlPost = "/HRM/OOF2020/DoPrintOrExport";
        ASOFT.helper.postTypeJson(urlPost, data, OOF2020.exportOrPrintSuccess);
    }

    this.exportOrPrintSuccess = function (result) {
        if (result) {
            if (result.isDataSource) {
                var urlPrint = '/HRM/OOF2020/ReportViewer';
                var urlExcel = '/HRM/OOF2020/GetReportFile';
