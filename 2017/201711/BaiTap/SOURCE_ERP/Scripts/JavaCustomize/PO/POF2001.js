...
$(document).ready(function () {
    ...
    GRID_AUTOCOMPLETE.config({
        ...
        setDataItem: function (selectedRowItem, dataItem) {
            ...
            selectedRowItem.model.set("WareHouseID", WareHouseID);        
        }
    });
    ...
    if ($("#isUpdate").val() == "True") {
        ...
        OpenComboDynamic($("#InventoryTypeID").data("kendoComboBox"));
...
function GetTime(actionTime) {
    ASOFT.helper.postTypeJson("/PO/POF2000/GetTime", {}, function (result) {
        if (actionTime == 1) {
            $("#EmployeeName").val(result.pofSetting.EmployeeName);
            $("#EmployeeID").val(result.pofSetting.EmployeeID);
            $("#PurchaseManName").val(result.EmployeeName);
            $("#PurchaseManID").val(result.EmployeeID);
            $("#VoucherTypeID").data("kendoComboBox").value(result.pofSetting.VoucherTypeID);
            $("#ClassifyID").data("kendoComboBox").value(result.pofSetting.ClassifyID);
            $("#OrderStatus").data("kendoComboBox").value(result.pofSetting.OrderStatus);
            $("#InventoryTypeID").data("kendoComboBox").value(result.pofSetting.InventoryTypeID);
            $("#CurrencyID").data("kendoComboBox").value(result.pofSetting.CurrencyID);
            $("#PaymentID").data("kendoComboBox").value(result.pofSetting.PaymentID);
            $("#ReceivedAddress").val(result.pofSetting.DeReAddress);
            $("#ExchangeRate").val(result.pofSetting.ExchangeRate);
            $("#Notes").val(result.pofSetting.Notes);
            WareHouseID = result.pofSetting.WareHouseID;
        }
    });
}
...
function receiveResult(result) {
    ...
    if (action == 3) {
        ...
        selectedItem.set("WareHouseID", WareHouseID);     
    }
}
