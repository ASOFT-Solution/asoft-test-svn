...
$(document).ready(function () {
    ...
    $(GridOT2001.tbody).on("change", "td", function (e) {
        ...
        if (column == 'cbb5097' && e.target.value != "") {
            var data = [];
...
function onCheckGridEdit(posGrid, key) {
    var data = posGrid.dataItem(posGrid.select());
    if ($(posGrid.select()).find("input#CbGridEdit_" + key + "[type='checkbox']").is(':checked')) {
        $(posGrid.select()).find("input#CbGridEdit_" + key + "[type='checkbox']").prop("checked", "checked");

        data[key] = 1;
    }
    else {
        $(posGrid.select()).find("input#CbGridEdit_" + key + "[type='checkbox']").prop('checked', '');
        data[key] = 0;
    }
