...
function getFormData() {
    ...
    var data = ASOFT.helper.dataFormToJSON("SF1016");
    return data;
}

function saveData() {
    if (ASOFT.form.checkRequired('SF1016')) {
        return;
    }
    //if (formIsInvalid()) {
    //    return;
    ...
    if (result.Status == 0) {
        ...
        switch (saveActionType) {
            case 1: // Trường hợp lưu & nhập tiếp
                formStatus = 1;
                ASOFT.form.displayInfo('#SF1016', ASOFT.helper.getMessage(result.Message).format(result.Data))
                parent.refreshGrid();
            ...
            case 2: // Trường hợp lưu & sao chép
                ...
                ASOFT.form.displayInfo('#SF1016', ASOFT.helper.getMessage(result.Message).format(result.Params));
                parent.refreshGrid();
                posGrid.dataSource.read();
            ...
            case 3: // Trường hợp lưu và đóng
                formStatus = 1;
                ASOFT.form.displayInfo('#SF1016', ASOFT.helper.getMessage(result.Message).format(result.Data.TypesSelected));
                ASOFT.helper.showErrorSeverOption(0, result, 'SF1016', function () {
                    ASOFT.asoftPopup.hideIframe(true);
                }, null, null, true);
                break;
            default:
        ...
        ASOFT.form.displayWarning('#SF1016', ASOFT.helper.getMessage(result.Message).format(result.Data).format(result.Data));
    }
}
