...
$(document).ready(function () {
    ...
    $("form#POSF0046 #rdoPeriod").change(function () {
    ...
    $("form#POSF0046 #rdoDate").change(function () {
        if ($(this).prop('checked')) {
            isPeriod = 0;
...
function getData() {
    ...
    var data = ASOFT.helper.getFormData(null, "POSF0046");
...
//Sự kiện đóng popup
function btnPOSF0046Close_Click(e) {
    ASOFT.asoftPopup.hideIframe(true);
}

//sự kiện in
function btnPOSF0046Print_Click() {
    isPrint = true;
    var data = getData();
    var url = $('#UrlDoPrint').val();
    ASOFT.helper.post(url, data, function (result) {
        ASOFT.ExportSuccessCommon.Load(result, "POS", "POSF0046", true);
    });
}

//Sự kiện xuất excel
function btnPOSF0046Export_Click() {
    isPrint = false;
    var data = getData();
    var url = $('#UrlDoPrint').val();
    ASOFT.helper.post(url, data, function (result) {
        ASOFT.ExportSuccessCommon.Load(result, "POS", "POSF0046", false);
    });
}

//function posf0046ExportSuccess(result) {
//    if (result) {
