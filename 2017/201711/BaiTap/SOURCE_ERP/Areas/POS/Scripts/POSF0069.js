...
$(document).ready(function () {
    ...
    btnChooseText = $($('#btnPOSF0069Choose').find('span')[0]).text();
    ...
    $("form#POSF0069 #rdoPeriod").change(function () {
    ...
    $("form#POSF0069 #rdoDate").change(function () {
        if ($(this).prop('checked')) {
            isPeriod = 0;
...
function getData() {
    ...
    var data = ASOFT.helper.dataFormToJSON("POSF0069");
...
//Sự kiện đóng popup
function btnPOSF0069Choose_Click(e) {
    var data = {};

    var postUrl = ASOFT.helper.renderUrl('/POS/POSF0069/POSF00691', data);

    // [2] Render iframe
    ASOFT.asoftPopup.showIframe(postUrl, {});
}

function btnPOSF0069UnChoose_Click(e) {
    recieveResult({});
}

//Sự kiện đóng popup
function btnPOSF0069Close_Click(e) {
...
function recieveResult(data) {
    ...
    if (selectedRows) {
        ...
        if (length > 0) {
            $($('#btnPOSF0069Choose').find('span')[0]).text(
                "( " + length + " ) " + btnChooseText);
        } else {
            $($('#btnPOSF0069Choose').find('span')[0]).text(btnChooseText);
        }
    }
...
//sự kiện in
function btnPOSF0069Print_Click() {
    isPrint = true;
    var data = getData();
    ...
    ASOFT.helper.postTypeJson(url, data, function (result) {
        ASOFT.ExportSuccessCommon.Load(result, "POS", "POSF0069", true);
    });
}

//Sự kiện xuất excel
function btnPOSF0069Export_Click() {
    isPrint = false;
    var data = getData();
    data.Keys = selectedKeys;
    ...
    var url = $('#UrlDoPrint').val();
    ASOFT.helper.postTypeJson(url, data, function (result) {
        ASOFT.ExportSuccessCommon.Load(result, "POS", "POSF0069", false);
    });
}

//function POSF0069ExportSuccess(result) {
//    if (result) {
