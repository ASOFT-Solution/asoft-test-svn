//####################################################################
//# Copyright (C) 2010-2011, ASoft JSC.  All Rights Reserved. 
//#
//# History:
//#     Date Time       Updater         Comment
//#     28/10/2014      Đức Quý         Tạo mới
//####################################################################

CIF1040 = new function () {
    this.isSearch = false;
    this.urlUpdate = null;
    this.CIF1040Grid = null;
    this.formStatus = null;

    // Filter button events
    this.btnFilter_Click = function () {
        ASOFT.form.clearMessageBox();
        CIF1040.isSearch = true;
        CIF1040.CIF1040Grid.dataSource.page(1);
    };

    // Reset filter button events
    this.btnClearFilter_Click = function () {
        ASOFT.form.clearMessageBox();
        $('#FormFilter input').val('');
        CIF1040.isSearch = false;
        CIF1040.CIF1040Grid.dataSource.page(1);
    };

    //Send data grid
    this.sendDataFilter = function () {
        var dataMaster = ASOFT.helper.dataFormToJSON("FormFilter");
        dataMaster['IsSearch'] = CIF1040.isSearch;
        return dataMaster;
    };

    // Add new button events
    this.btnAddNew_Click = function () {
        // [1] Tạo form status : Add new
        CIF1040.formStatus = 1;
        CIF1040.urlUpdate = $('#UrlInsert').val();

        // [2] Url load dữ liệu lên form
        var postUrl = $("#UrlCIF1041").val();

        // [3] Data load dữ liệu lên form
        var data = {
            FormStatus: CIF1040.formStatus
        };

        // [4] Hiển thị popup
        CIF1041.showPopup(postUrl, data);
    };

    // Edit button events
    this.btnEdit_Click = function () {
        // [1] Tạo form status : Edit
        CIF1040.formStatus = 2;
        CIF1040.urlUpdate = $('#UrlUpdate').val();
        var templateID = $('#TemplateID').text();

        ASOFT.helper.post($('#UrlCheckEdit').val(), { TemplateID: templateID }, function (result) {
            if (result.length == 0) {
                // [2] Url load dữ liệu lên form
                var postUrl = $("#UrlCIF1041").val();

                // [3] Data load dữ liệu lên form
                var data = {
                    FormStatus: CIF1040.formStatus,
                    TemplateID: templateID
                };

                // [4] Hiển thị popup
                CIF1041.showPopup(postUrl, data);
            }
            else {
                ASOFT.form.displayMessageBox('#ViewMaster', [ASOFT.helper.getMessage(result[0].MessageID)]);
            }
        });
    };

    // Delete button events
    this.btnDelete_Click = function () {
        var urlDelete = $('#UrlDelete').val();
        var args = [];
        var data = { };
        if (CIF1040.CIF1040Grid) { // Lấy danh sách các dòng đánh dấu
            var records = ASOFT.asoftGrid.selectedRecords(CIF1040.CIF1040Grid, 'FormFilter');
            if (records.length == 0) return;
            for (var i = 0; i < records.length; i++) {
                args.push(records[i].TemplateID);
            }
        } else {
            // Lấy đối tượng hiện tại nếu đang ở màn hình details
            var templateID = $('#TemplateID').text();
            args.push(templateID);
        }

        ASOFT.dialog.confirmDialog(ASOFT.helper.getMessage('00ML000024' /*A00ML000002*/), function () {
            data['args'] = args;
            ASOFT.helper.postTypeJson(urlDelete, data, CIF1040.deleteSuccess);
        });
    };

    //Kết quả server trả về sau khi xóa
    this.deleteSuccess = function (result) {
        var formId = null;
        var displayOnRedirecting = null;

        if (document.getElementById('FormFilter')) {
            formId = "FormFilter";
            displayOnRedirecting = false;
        } else if (document.getElementById('ViewMaster')) {
            formId = "ViewMaster";
            displayOnRedirecting = true;
        }

        ASOFT.helper.showErrorSeverOption(1, result, formId, function () {
            var urlCIF1040 = $('#UrlCIF1040').val();
            // Chuyển hướng hoặc refresh data
            if (urlCIF1040 != null) {
                window.location.href = urlCIF1040; // redirect index
            }
        }, null, null, true, displayOnRedirecting, "FormFilter");

        if (CIF1040.CIF1040Grid) {
            CIF1040.CIF1040Grid.dataSource.page(1); // Refresh grid 
        }
    };

    //Hiển thị các dòng được chọn
    this.showRecord = function () {
        CIF1040.showHideRecords(0);
    };

    //Ẩn các dòng được chọn
    this.hideRecord = function () {
        CIF1040.showHideRecords(1);
    };

    this.showHideRecords = function (disabled) {
        var args = [];
        var records = ASOFT.asoftGrid.selectedRecords(CIF1040.CIF1040Grid, 'FormFilter');
        if (records.length == 0) return;
        for (var i = 0; i < records.length; i++) {
            args.push(records[i].TemplateID);
        }

        var url = $("#UpdateRecord").val();
        var data = {
            args: args,
            disabled: disabled
        };

        ASOFT.helper.postTypeJson(url, data, function (result) {
            ASOFT.helper.showErrorSeverOption(1, result, "FormFilter", function () {
                CIF1040.CIF1040Grid.dataSource.page(1);
            }, null, null, true, false, "FormFilter");
        });
    };
}

$(document).ready(function () {
    CIF1040.CIF1040Grid = ASOFT.asoftGrid.castName('CIF1040Grid');
})