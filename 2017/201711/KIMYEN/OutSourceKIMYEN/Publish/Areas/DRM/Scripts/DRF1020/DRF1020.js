...
DRF1020 = new function () {
    this.isSearch = false;
    this.DRF1020Grid = null;
    this.urlUpdate = null;
    ...
    this.btnFilter_Click = function () {
        ...
        DRF1020.isSearch = true;
        DRF1020.DRF1020Grid.dataSource.page(1);
    };
    ...
    this.btnClearFilter_Click = function () {
        ...
        DRF1020.isSearch = false;
        DRF1020.DRF1020Grid.dataSource.page(1);
    };

    this.sendDataFilter = function () {
        var dataMaster = ASOFT.helper.dataFormToJSON("FormFilter");
        dataMaster['IsSearch'] = DRF1020.isSearch;
        return dataMaster;
    }
    ...
    this.btnAddNew_Click = function () {
        ...
        DRF1020.formStatus = 1;
        DRF1020.urlUpdate = $('#UrlInsert').val();

        // [2] Url load dữ liệu lên form
        var postUrl = $("#UrlDRF1021").val();

        // [3] Data load dữ liệu lên form
        var data = {
            FormStatus: DRF1020.formStatus
        };

        // [4] Hiển thị popup
        DRF1021.showPopup(postUrl, data);
    };

    // Edit button events
    this.btnEdit_Click = function () {
        // [1] Tạo form status : Edit
        DRF1020.formStatus = 2;
        DRF1020.urlUpdate = $('#UrlUpdate').val();
        var infoID = $('td#InfoID').text();

        ASOFT.helper.post($('#UrlCheckEdit').val(), { InfoID: infoID }, function (result) {
            if (result.length == 0) {
                // [2] Url load dữ liệu lên form
                var postUrl = $("#UrlDRF1021").val();
    ...
    this.btnDelete_Click = function () {
        ...
        var args = [];
        var data = {};
        if (DRF1020.DRF1020Grid) { // Lấy danh sách các dòng đánh dấu
            var records = ASOFT.asoftGrid.selectedRecords(DRF1020.DRF1020Grid, 'FormFilter');
            if (records.length == 0) return;
            for (var i = 0; i < records.length; i++) {
                args.push(records[i].InfoID);
            }
        } else {
        ...
        ASOFT.dialog.confirmDialog(ASOFT.helper.getMessage('00ML000024' /*A00ML000002*/), function () {
            ...
            ASOFT.helper.postTypeJson(urlDelete, data, DRF1020.deleteSuccess);
        });
    };
    ...
    this.deleteSuccess = function (result) {
        ...
        ASOFT.helper.showErrorSeverOption(1, result, formId, function () {
            var urlDRF1020 = $('#UrlDRF1020').val();
            // Chuyển hướng hoặc refresh data
            if (urlDRF1020 != null) {
                window.location.href = urlDRF1020; // redirect index
            }
        }, null, null, true, displayOnRedirecting, "FormFilter");

        if (DRF1020.DRF1020Grid) {
            DRF1020.DRF1020Grid.dataSource.page(1); // Refresh grid 
        }
    }

    //Hiển thị các dòng được chọn
    this.showRecord = function () {
        DRF1020.showHideRecords(0);
    };

    //Ẩn các dòng được chọn
    this.hideRecord = function () {
        DRF1020.showHideRecords(1);
    };
...
$(document).ready(function () {
    DRF1020.DRF1020Grid = ASOFT.asoftGrid.castName('DRF1020Grid');
})