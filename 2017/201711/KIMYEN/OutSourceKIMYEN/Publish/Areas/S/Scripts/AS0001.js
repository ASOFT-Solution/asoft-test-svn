...
$(document).ready(function () {
    ...
    $("a#hrmf0001").click(function () {
        url = $('#UrlHRMF0001').val();
        showSFPopup(url);
    });

    $("a#hrmf0010").click(function () {
        url = $('#UrlHRMF0010').val();
        showSFPopup(url);
    });
    ...
    $("a#LMF0001Popup").click(function () {
        url = $("#UrlLMF0001").val();
        showSFPopup(url);
    });

    $("a#LMF0002Popup").click(function () {
        url = $("#UrlLMF0002").val();
        showSFPopup(url);
    });

    $("#closeBook_LM").click(closeBookLM);
    $("#openBook_LM").click(openBookLM);

   
    $('#periodClosePopup').click(closeBookPOS);
    $('#periodOpenPopup').click(openBookPOS);

    // CSM
    $("a#CSMF0001Popup").click(function () {
        url = $("#UrlCSMF0001").val();
        showSFPopup(url);
    });

    $("a#CSMF0002Popup").click(function () {
        url = $("#UrlCSMF0002").val();
        showSFPopup(url);
    });

    $('#periodClosePopupPO').click(closeBookPO);
    $('#periodOpenPopupPO').click(openBookPO);
});

function openBookPO() {
    var data = {};
    var url = "/CloseOpenBook/OpenBook?ModuleID=PO";
    ASOFT.helper.post(url, data, openCloseBookSuccessCommon);
}

function closeBookPO() {
    url = "/CloseOpenBook?ModuleID=PO";
    data = {
...
function openCloseBookSuccessCommon(result) {
    if (result.Status == 0) {
        if (result.Data) {
...
function openBookPOS() {
    var data = {};
    var url = "/CloseOpenBook/OpenBook?ModuleID=POS";
    ASOFT.helper.post(url, data, openCloseBookSuccessCommon);
}
...
* Khóa sổ kỳ kế toán
*/
function closeBookPOS() {
    url = "/CloseOpenBook?ModuleID=POS";
    data = {
        CloseBookFunctionJS: "btnCloseBook",
        CloseFunctionJS: "btnClosePopup"
    };
...
* Mo so LM
*
* [Kim Vu] Create New [22/01/2018]
**/
function openBookLM() {
    var data = {};
    var url = "/CloseOpenBook/OpenBook?ModuleID=LM";
...
* [Kim Vu] Create New [22/01/2018]
**/
function closeBookLM() {
    url = "/CloseOpenBook?ModuleID=LM";
    data = {
        CloseBookFunctionJS: "btnCloseBook",
        CloseFunctionJS: "btnClosePopup"
    };
