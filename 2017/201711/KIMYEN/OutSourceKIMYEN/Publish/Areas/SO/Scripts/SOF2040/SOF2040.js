...
function renderWorkdate(userid, from_date, to_date) {
    var xhttp = $.get('/SO/SOF2040/UserWorkDay?userid=' + userid + "&fromdate=" + from_date + "&todate=" + to_date, function (data, status) {
        var salemandates = $.parseJSON(data);

        $.each(salemandates, function (index, data) {
            data.DateTime = xdatetime(data.DateTime, 'date');
            data.StartDateTime = xdatetime(data.StartDateTime, 'time');
            data.EndDateTime = xdatetime(data.EndDateTime, 'time');
        });

        ASOFT.helper.post($("#UrlSearch").val(),
                          { status: status, check: salemandates.length > 0 ? true : false, data: JSON.stringify(salemandates) }, function (data) {
                              $('#salemandates').html(data);
                          });
        
    }).error(function () {
        ASOFT.helper.post($("#UrlSearch").val(),
                          { status: 'fail', check:false ,data: [] }, function (data) {
                                    $('#salemandates').html(data);
                          });
    });
}
    ...
    if (datas.length > 0) {
        for (var i = 0; i < datas.length; i++) {
            ...
            if (i > 0) {
                ...
                if ( distance > betw ||  i == (datas.length-1) || datas[i].Name != '') {
                    local = { lat: datas[i].Latitude, lng: datas[i].Longitude };
                    locals.push(datas[i]);
                }
            }
...
function loopDraw(data, i, length) {
    ...
    if (i > 0) {
        ...
        //location = { position: placeLatlng, description: "" };
    	//addMarkerPlace(location, iconPlace);
