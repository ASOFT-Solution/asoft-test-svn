    ...
    if (datas.length > 0) {
        ...
        for (var i = 0; i < datas.length; i++) {
            ...
            if (i > 0) {
                ...
                if ((distance < 1000 || distance > 5000) && data.Name == "") {
                //if ( distance < 800 ) {
                    continue;
                } else {
					local = { lat: data.Latitude, lng: data.Longitude };
			...
			var mdh = '';
			if(data.Name != ""){
				mdh = '<p class="address"><i class="fa"></i>Mã đơn hàng: ' + data.OrderNo + '</p>';
			}
			...
			if(coun == datas.length && data.Name == ''){
				name = 'Điểm kết thúc';
			}
			
            if (data.Name != "" || i == 0 || coun == datas.length) {
                // Make description content
                description = '<div class="description">'
