...
var shopName = null;
var rowNumber = 0;
var rowNumberManager = 0;
var posGrid = null;
var posGridManager = null;
var example = null;
...
$(document).ready(function () {
    ...
    posGridManager = $("#GridManager").data("kendoGrid");
    ...
    if (posGridManager) {
        posGridManager.bind("dataBound", function (e) {
            rowNumberManager = 0;

            $("#GridManager").find('td').on("focusin", function (e) {
                var datauid = e.currentTarget.parentElement.attributes["data-uid"].value;
                if (datauid != apk) {
                    posGridManager.closeCell();
                }
            })

            $("#GridManager .k-grid-content").css("height", "250px");
            $("#GridManager").css("max-height", "300px");
            $("#GridManager").attr("AddNewRowDisabled", "false");
        });
    }
    ...
    var cb3 = $('#PackagePriceID').data('kendoComboBox');
    $('#IsPackage').bind('change', function (e) {
        $("input[name=IsPackage]").val($(this).val());
        cb3.enable($(this).prop('checked'));
    }).trigger('change');

    var cb4 = $('#DisplayWareHouseID').data('kendoComboBox');
    $('#IsDisplay').bind('change', function (e) {
        cb4.enable($(this).prop('checked'));
    });

    var cb5 = $('#BrokenWareHouseID').data('kendoComboBox');
    $('#IsBroken').bind('change', function (e) {
        cb5.enable($(this).prop('checked'));
    });
...
function renderNumberManager(data) {
    return ++rowNumberManager;
}
...
function getFormData() {
    ...
    data.IsDisplay = ($("#IsDisplay").is(":checked"));
    data.IsBroken = ($("#IsBroken").is(":checked"));
    ...
    if (posGridManager) {
        data.DetailManager = posGridManager.dataSource.data();
    }
    ...
    data.IsPromote = ($("#IsPromote").is(":checked"));
    data.IsPromote1 = ($("#IsPromote1").is(":checked"));
    data.IsPackage = $("#IsPackage").is(":checked");
    return data;
}
    ...
    if (result.Status == 0) {
        ...
        switch (saveActionType) {
            case 1: // Trường hợp lưu & nhập tiếp
                ...
                if (posGridManager) {
                    posGridManager.dataSource.data([]);
                }
                if (result.Data == 1) {
                    //parent.reloadWindow();
            ...
            case 3: // Trường hợp lưu và đóng
                ...
                posGrid.dataSource.read();
                if (posGridManager) {
                    posGridManager.dataSource.read();
                }
...
function clearFormPOSF00101() {
    ...
    var comboboxVoucherType13 = $("#VoucherType13").data("kendoComboBox");
    comboboxVoucherType13.value('');

    var comboboxVoucherType14 = $("#VoucherType14").data("kendoComboBox");
    comboboxVoucherType14.value('');

    var comboboxVoucherType15 = $("#VoucherType15").data("kendoComboBox");
    comboboxVoucherType15.value('');

    var comboboxVoucherType16 = $("#VoucherType16").data("kendoComboBox");
    comboboxVoucherType16.value('');

    var comboboxVoucherType17 = $("#VoucherType17").data("kendoComboBox");
    comboboxVoucherType17.value('');

    var comboboxVoucherType18 = $("#VoucherType18").data("kendoComboBox");
    comboboxVoucherType18.value('');
...
function saveDataUpdate(e) {
    ...
    if (checkGridAnyCheck()) {
        return;
    }

    if (isDuplicateWareHouse()) {
        return false;
    }

    if (isDuplicateVoucherTypeInfo()) {
        return false;
    }
...
function formIsInvalid() {
    ...
    if (posGridManager) {
        if (posGridManager.dataSource.data().length > 0) {
            if (ASOFT.asoftGrid.editGridValidate(posGridManager, ['APK', 'IsUsed', 'ShopID'])) {
            ...
            $('#GridManager').addClass('asf-focus-input-error');
            //display message
    ...
    var check = false;
    var data = ASOFT.helper.dataFormToJSON("POSF00101");
    if (data.WareHouseID == data.ComWarehouseID) {
        check = comboInvalid = true;
        addError("WareHouseID");
        addError("ComWarehouseID");
    }

    if (data.DisplayWareHouseID) {
        if(data.WareHouseID == data.DisplayWareHouseID)
        {
            addError("DisplayWareHouseID");
            addError("WareHouseID");
            check = comboInvalid = true;
        }
        if (data.ComWarehouseID == data.DisplayWareHouseID)
        {
            addError("DisplayWareHouseID");
            addError("ComWarehouseID");
            check = comboInvalid = true;
    ...
    if (ASOFTEnvironment.CustomerIndex.OKIA == "True") {
        if (data.BrokenWareHouseID) {
            if (data.WareHouseID == data.BrokenWareHouseID) {
                addError("WareHouseID");
                addError("BrokenWareHouseID");
                check = comboInvalid = true;
            }
            if (data.ComWarehouseID == data.BrokenWareHouseID) {
                addError("ComWarehouseID");
                addError("BrokenWareHouseID");
                check = comboInvalid = true;
            }
        ...
        if (data.BrokenWareHouseID && data.DisplayWareHouseID) {
            if (data.BrokenWareHouseID == data.DisplayWareHouseID) {
                addError("BrokenWareHouseID");
                addError("DisplayWareHouseID");
                check = comboInvalid = true;
        ...
        if (check) {
            msg = ASOFT.helper.getMessage('POSFML000104');
            ASOFT.form.displayError('#' + ID, msg);
        }
    }
...
function refreshDefaultViewModel() {
    ...
    defaultViewModel.Disabled = ($("#Disabled").is(":checked"));
    defaultViewModel.IsTable = ($("#IsTable").is(":checked"));
    defaultViewModel.IsDiscount = ($("#IsDiscount").is(":checked"));
        ...
        if (result.Message != "") {
            ...
            if (result.Data && result.Data.length > 0) {
                ...
                for (var i = 0; i < result.Data.length; i++) {
                    var m = ASOFT.helper.getMessage(result.Data[i].MessageID).format(result.Data[i].Params);
                    lmsg.push(m);
                }
                ASOFT.form.displayWarning('#POSF00101', lmsg);
            }
        }
    }
}
...
function receiveResult(result) {
    ...
    posGridManager.dataSource.add(result);
}

function setIsUsed(data) {
    if (data.IsUsed == 1) {
        apk = data.uid;
        isUsed = 1;
    }
...
function radio_Click(dataapk) {
    var length = posGridManager.dataSource._data.length;
    for (var i = 0; i < length; i++) {
        var item = posGridManager.dataSource.at(i);
        if (item.uid == dataapk) {
            item.set("IsUsed", 1);
        }
        else {
...
function genDeleteBtn(data) {
    if (data.IsUsed == 1)
        return "";
    return "<a onclick=deleteDetail_Click(this,'" + data.EmployeeID + "','" + data.APK + "') class='asf-i-delete-24 asf-icon-24'  onsubmit = 'return false'><span>Del</span></a>";
}

function deleteDetail_Click(e, EmployeeID, APK) {
    ASOFT.form.clearMessageBox();
    if (APK != "undefined") {
        ASOFT.helper.postTypeJson("/POS/POSF0010/CheckManager", { EmployeeID: EmployeeID }, function (result) {
            if (result) {
                return deleteItems(e);
            }
            else {
                ASOFT.form.displayWarning('#POSF00101', ASOFT.helper.getMessage("POSFML000086").format(EmployeeID));
                return false;
            }
        });
    }
...
function deleteItems(tagA) {
    //remove gridEdit
    ASOFT.asoftGrid.removeEditRow(tagA, posGridManager);
    return false;
}
...
function isDuplicateVoucherTypeInfo() {
    ...
    if (isInValid) ASOFT.form.displayMessageBox("form#" + "POSF00101", [ASOFT.helper.getMessage("POSFML000095")]);

    return isInValid;
}


function isDuplicateWareHouse() {

    var comboWareHouseId = $("#WareHouseID").data("kendoComboBox");
    var comboComWareHouseId = $("#ComWarehouseID").data("kendoComboBox");

    if (comboWareHouseId.value() == comboComWareHouseId.value()) {
        resetMessageBox("#POSF00101");
        addError("WareHouseID");
        addError("ComWarehouseID");
        var errorMessage = kendo.format(ASOFT.helper.getMessage("POSFML000103"), $("label[for=" + "WareHouseID" + "]").text(), $("label[for=" + "ComWarehouseID" + "]").text())
        ASOFT.form.displayMessageBox("form#" + "POSF00101", [errorMessage]);
        return true;
    }
    return false;
