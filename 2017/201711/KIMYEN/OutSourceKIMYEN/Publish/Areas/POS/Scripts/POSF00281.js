...
function createViewModel() {
    posViewModel = kendo.observable({
        ...
        isInvalid: function () {
            ...
            if (!check && ASOFTEnvironment.CustomerIndex.OKIA == "True") {
                var lengthGrid = posGrid.dataSource._data.length;
                for (var i = 0; i < lengthGrid; i++) {
                    var item = posGrid.dataSource.at(i);
                    if (item.ShipQuantity == 1 && (item.SerialNo == "" || item.SerialNo == null)) {
                        var row = $(posGrid.tbody).find('tr')[i];
                        var indexCell = $($("#POSF0028Grid").find("th[data-field='SerialNo']")).index();
                ...
                if (check) {
                    ...
                    ASOFT.form.displayError("#POSF00281", kendo.format(msg, $($("#POSF0028Grid").find("th[data-field='SerialNo']")).attr("data-title")));
                }
            }
            ...
            if (!check && $("#isCLOUD").val() == "True") {
                if (this.gridDataSource.data().length <= 0) {
                    ...
                    //show quantity
                    if (ASOFT.asoftGrid.editGridValidate(posGrid, ['Description', 'SerialNo'])) {
...
function onInsertSuccess(result) {
    if (result.Status == 0) {
        switch (action) {
            case 1://save new      
                if (result.Data.id) {
                    for (var i = 0; i < result.Data.id.length; i++) {
                        Print(result.Data.id[i]);
                    }
                }
...
function noChangeSerialNo() {
    posGrid.bind('dataBound', function (e) {
        $("#POSF0028Grid").find('td').on("focusin", function (e) {
            var row = e.target.closest("tr");
            var data = posGrid.dataItem(row);
            var index = e.delegateTarget.cellIndex;
            var th = $($("#POSF0028Grid").find('th')[index]).attr("data-field");
            if (th == 'SerialNo' && data.ShipQuantity > 1) {
                posGrid.closeCell();
            }
        })
    })
...
function Grid_Save(e) {
    ...
    if (e.values.ShipQuantity != null && e.values.ShipQuantity != "")
    {
        if (e.values.ShipQuantity > 1)
        {
