...
$(window).on('load', function () {
    $("#image-Load").delay(500).fadeOut('slow');
})
...
function attachFile_Click() {
    var divisionAdd = $(".DivisionID").text() == "@@@" ? $("#EnvironmentDivisionID").val() : $(".DivisionID").text();
    var urlLink = "/AttachFile?Type=1&DivisionID=" + divisionAdd + "&RelatedToTypeID=" + parseInt($(".RelatedToTypeID").text()) + "&RelatedToID=" + $("#PK").val() + "&TableID=" + $("#sysTable").val();
    ASOFT.asoftPopup.showIframe(urlLink, {});
}
...
function addBrowseReviews_Click() {
    if(id == 'KPIF2002')
        var urlLink = "/PopupMasterDetail/Index/KPI/KPIF2001?PK=" + $("#PK").val() + "&Table=KPIT20001&key=APK&IsView=1";
    else
        var urlLink = "/PopupMasterDetail/Index/PA/PAF2001?PK=" + $("#PK").val() + "&Table=PAT20001&key=APK&IsView=1";
    ASOFT.asoftPopup.showIframe(urlLink, {});
}
//-------------Action LM------------------
//Phong tỏa
function LockAdvanceAccount_Click(){
    var urlLink = "/PopupMasterDetail/Index/LM/LMF2011?AdvanceTypeID=0&ScreenID=LMF2002&VoucherNo=" + $('.VoucherNo').text();
    ASOFT.asoftPopup.showIframe(urlLink, {});
}
//Giải tỏa
function RelieveAdvanceAccount_Click(){
    var urlLink = "/PopupMasterDetail/Index/LM/LMF2011?AdvanceTypeID=1&ScreenID=LMF2002&VoucherNo=" + $('.VoucherNo').text();
    ASOFT.asoftPopup.showIframe(urlLink, {});
}
//Giải ngân
function Disburse_Click(){
    var urlLink = "/PopupLayout/Index/LM/LMF2021?&ScreenID=LMF2002&VoucherNo=" + $('.VoucherNo').text();
    ASOFT.asoftPopup.showIframe(urlLink, {});
}
//Thanh toán
function Payment_Click(){
    var urlLink = "/PopupLayout/Index/LM/LMF2031?&ScreenID=LMF2002&VoucherID=" + $("#PK").val() + "&VoucherNo=" + $('.VoucherNo').text();
    ASOFT.asoftPopup.showIframe(urlLink, {});
}
//Điều chỉnh
function Adjust_Click(){
    var urlLink = "/PopupLayout/Index/LM/LMF2041?&ScreenID=LMF2002&VoucherID=" + $("#PK").val() + "&VoucherNo=" + $('.VoucherNo').text();
    ASOFT.asoftPopup.showIframe(urlLink, {});
}
//Tất toán
function Finish_Click() {
    ASOFT.helper.postTypeJson("/LM/LMF2000/UpdateLMT2001", { VoucherID: $("#PK").val() }, function (result) {
        if (result) {
            $('a[id^="StatusNo"]').children().removeClass('stageSelect');
            $('a[id="StatusNo9"]').children().addClass('stageSelect');

            $('a[id^="StatusNo"]').children().children().removeClass('arrowSelect');
            $('a[id="StatusNo9"]').children().children().addClass('arrowSelect');
        }
    });
}
//Lịch trả nợ
function PaymentPlan_Click() {
    var urlLink = "/PopupLayout/Index/LM/LMF2024?&ScreenID=LMF2022&VoucherID=" + $("#PK").val() + "&VoucherNo=" + $('.VoucherNo').text();
    ASOFT.asoftPopup.showIframe(urlLink, {});
}
//----------------------------------------
...
function keydowCommon(e, element) {
    if (e.keyCode < 48 || (e.keyCode > 57 && e.keyCode < 96) || e.keyCode > 105) {
        if (e.keyCode != 9 && e.keyCode != 13 && e.keyCode != 37 && e.keyCode != 39 && e.keyCode != 189 && e.keyCode != 109 && e.keyCode != 8 && ((e.keyCode != 190 && e.keyCode != 110) || ($(element).val()).indexOf('.') != -1)) {
            e.preventDefault()
        }
    }
}

function formatConvertCommon(value) {
...
$(document).ready(function () {
    ...
    if ($("#A00_SystemInfo").length > 0) {
        $("#GR_HeThong-1").append($("#A00_SystemInfo-1 .asf-master-content"));
    ...
    setTimeout(function () {
        if ($('ul.k-tabstrip-items').length > 0) {
            ...
            var tabStrip = $('#' + id).data("kendoTabStrip");
            if (tabStrip != null) {
                var ul = tabStrip.wrapper.children(".k-tabstrip-items");
...
ASOFT.asoftGrid = new function () {
    ...
    this.setHeight = function (grid) {
        ...
        if (ASOFTEnvironment.CustomerIndex.XUONGRONG == "True" || window.innerWidth < 1280) {
    ...
    this.checkAll = function (e) {
        ...
        if (gridDOM != undefined) {
            ...
            if (grid._group) {
                checkDatas = this.getDataCheckedGroup(grid);
            }
            else {
    ...
    //Hàm check All Record
    this.getDataCheckedGroup = function (grid) {
        var checkDatas = [];
        if (grid != undefined) {
            var id = grid.element.prop('id');
            var sector = kendo.format("#{0} {1}", id, ".asoftcheckbox:checked");
    ...
    this.setValueTextboxGroup = function (name, e, cellindex, rowindex) {
        var grid = ASOFT.asoftGrid.castName(name);
        var fielid = grid.columns[cellindex].field;
        if (fielid != undefined && $(grid.tbody).find('#' + fielid).val() != undefined) {
            var dateFormat = /^(\d{2})\/(\d{2})\/(\d{4})$/; //check value is a date
            ...
            for (var i = 0; i < dataGrid.length; i++)
            {
                ...
                if (dataRow.items)
                {
                    for (var j = 0; j < dataRow.items.length; j++) {
                        if (rowTemp == rowindex)
                        {
                            row = dataRow.items[j];
                            break;
    ...
    this.checkEditCell = function (model, fieldId) {
        if (model != undefined) {
            ...
            if (attr && attr.editable != undefined && attr.style != "display:none") {
                return true;
            }
        }
        return false;
    ...
    this.moveCell = function (elm, name) {
        ...
        var prevrow = grid.tbody.find('tr').eq(grid._group ? ASOFT.asoftGrid.currentRowGroup : ASOFT.asoftGrid.currentRow);
        
        //lấy cell theo index cell
        var cell = $(prevrow).find('td').eq(grid._group ? ASOFT.asoftGrid.currentCell + 1 : ASOFT.asoftGrid.currentCell);
        //Chọn dòng
        grid.select(prevrow);
        //Edit cell
        grid.editCell(cell);
    ...
    this.previousCell = function (elm, name, isEnter) {
        ...
        if (grid._group)
        {
           ASOFT.asoftGrid.setValueTextboxGroup(
               name,
               elm,
               ASOFT.asoftGrid.currentCell,
               ASOFT.asoftGrid.currentRow
        ...
        if (ASOFT.asoftGrid.currentCell == this.firstCell) {
            if (isEnter) {
                ...
                if (grid._group)
                {
                    ASOFT.asoftGrid.currentRowGroup -= 1;
                    var prevrow = grid.tbody.find('tr').eq(ASOFT.asoftGrid.currentRowGroup);
                    if (prevrow && !prevrow.attr("data-uid"))
                    {
                        ASOFT.asoftGrid.currentRowGroup -= 1;
                    }
                    prevrow = grid.tbody.find('tr').eq(ASOFT.asoftGrid.currentRowGroup);
                    if (prevrow && !prevrow.attr("data-uid")) {
                        ASOFT.asoftGrid.currentRowGroup -= 1;
                    }
                }
    ...
    this.nextCell = function (elm, name, isEnter) {
        ...
        if (grid._group)
        {
            ...
            {
                maxRows = maxRows + grid.dataSource._data[i].items.length;
            }
        }
        else {
            maxRows = grid.dataSource.data().length;
        }
        if (ASOFT.asoftGrid.currentCell == this.lastCell) {
            if (isEnter) {
                if (ASOFT.asoftGrid.currentRow == (maxRows - 1)) {
    ...
    this.editGridEdit = function (e) {
        ...
        var columns = grid.columns;
        var alowEdit = true;
        var countGroup = grid._group ? 1 : 0;
        var cellIndex = $(e.container).index() - countGroup;
    ...
    this.editGridDataBound = function (e) {
        ...
        var name = grid.element.prop('id');
        var columns = grid.columns;
        var lTr = grid.tbody.find('tr');
        for (var i = 0; i < lTr.length; i++) {
            if ($(lTr[i]).attr('data-uid') != undefined) {
                //$(lTr[i]).removeClass("k-alt");
                lTd = $(lTr[i]).find('td');
                for (var j = 0; j < columns.length; j++) {
                    var td = !grid._group ? lTd[j] : lTd[j + 1];
                    var column = columns[j];
                    var attr = column.attributes;
                    if (attr && attr.editable == "true") {
                        $(td).removeClass("asf-disable-td");
                    }
                    else {
                        $(td).addClass("asf-disable-td");
                    }
                }
            }
        }
        ...
        $(grid.tbody).off("keydown mouseleave", "td").on("keydown mouseleave", "td", function (e) {
            ...
            if (grid._group) {
                ...
                var dataGridGrow = grid.dataItem($(this).parent());
                if (dataGridGrow) {
                    for (var j = 0; j < dataGrid.length; j++) {
                        if (dataGrid[j].items) {
                            for (var k = 0; k < dataGrid[j].items.length; k++) {
                                if (dataGrid[j].items[k].uid == dataGridGrow.uid) {
                                    isRow = true;
                                    break;
                                }
                                d += 1;
                    ...
                    ASOFT.asoftGrid.currentRow = d;
                    ASOFT.asoftGrid.currentCell = $(this).index() - 1;
                    ASOFT.asoftGrid.currentRowGroup = $(this).parent().index();
                }
                else {
                    ASOFT.asoftGrid.currentCell = -1;
                }
            }
            else {
            ...
            if (ASOFT.asoftGrid.currentCell != -1) {
    ...
    this.editGridValidateNoEditGroup = function (grid, notRequiredFields, checkFunction) {
        var columns = grid.columns;
        var isError = false;
        $(grid.tbody).find('td').removeClass('asf-focus-input-error');
        $(grid.tbody).find("td").each(function (index, element) {
            if ($(element).parent().attr('data-uid')) {
                var dataGridGrow = grid.dataItem($(element).parent());
                var cellIndex = $(element).index() - 1;

                if (cellIndex != -1 && dataGridGrow) {
                    var editor = columns[cellIndex].editor;
                    if (editor != undefined) {
                        var column = columns[cellIndex];
                        if ($.inArray(column.field, notRequiredFields) == -1 && column.field != "") {
                            var row = dataGridGrow;
                            var value = row[column.field];
                            if (value === null || value === '' || value === undefined) {
                                isError = true;
                                $(element).addClass('asf-focus-input-error');
...
ASOFT.helper = new function () {
    ...
    this.getLanguageString = function (idLanguage, formID, area) {
        var URLGETLANGUAGE = "/GridCommon/GetLanguage";
        var strL = '';
        this.postTypeJson(URLGETLANGUAGE,
            { ID: idLanguage, FormID: formID, Area: area },
            function(result) {
                strL = result;
            });
        return strL;
    ...
    this.getLanguageStringA00 = function (idLanguage) {
        var URLGETLANGUAGE = "/GridCommon/GetLanguageA00";
        var strL = '';
        this.postTypeJson(URLGETLANGUAGE,
            { ID: idLanguage },
            function (result) {
