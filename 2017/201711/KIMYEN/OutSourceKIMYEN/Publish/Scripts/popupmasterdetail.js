...
$(document).ready(function () {
    ...
    if (tableName.length > 1) {
        $.each(tableName, function (index, value) {
            ...
            if (grid) {
                ...
                for (var i = 0 ; i < grid.columns.length; i++) {
                    if ($("#" + grid.columns[i].field).length > 0) {
                        grid.columns[i].editor = grid.columns[i].editor.replace('id="' + grid.columns[i].field + '"', 'id="' + grid.columns[i].field + value + '"');
                        grid.columns[i].editor = grid.columns[i].editor.replace('"#' + grid.columns[i].field + '"', '"#' + grid.columns[i].field + value + '"');
                    }
                }
            }
        })
        ...
        if (grid) {
            ...
            for (var i = 0 ; i < grid.columns.length; i++) {
                if ($("#" + grid.columns[i].field).length > 0) {
                    grid.columns[i].editor = grid.columns[i].editor.replace('id="' + grid.columns[i].field + '"', 'id="' + grid.columns[i].field + tableName[0] + '"');
                    grid.columns[i].editor = grid.columns[i].editor.replace('"#' + grid.columns[i].field + '"', '"#' + grid.columns[i].field + tableName[0] + '"');
                }
            }
        }
    }
...
function getDetail(tb) {
    ...
    if (Grid) {
        ...
        $.each(dataPost, function (key, value) {
            if (key != "item.TypeCheckBox" && key != "Unique" && dataPost[key + "_Content_DataType"] != undefined) {
                if (key.indexOf("_Content_DataType") == -1 && key.indexOf("_Type_Fields") == -1 && key.indexOf("tableNameEdit") == -1 && key.indexOf("listRequired") == -1 && key != "CheckInList" && key.indexOf("CbGridEdit_") == -1 && key.indexOf("_" + tb) != -1) {
                    if (value == "false")
                        value = "0";
                    if (value == "true")
                        value = "1";
                    value1[key.replace("_" + tb, "")] = value;
                    if (defaultValue && value != defaultValue[key])
                    {
                        HistoryChange = 1;
                    }
                }
    ...
    if (typeof CustomInsertPopupMaster === 'function') {
        ...
        $.each(data, function (key, value) {
            if (key != "item.TypeCheckBox" && key != "Unique" && data[key + "_Content_DataType"] != undefined) {
                if (key.indexOf("_Content_DataType") == -1 && key.indexOf("_Type_Fields") == -1 && key.indexOf("tableNameEdit") == -1 && key.indexOf("listRequired") == -1 && key != "CheckInList" && key.indexOf("CbGridEdit_") == -1 && tableNameCh.indexOf(key.split('_')[key.split('_').length - 1]) == -1) {
                    if (value == "false")
                        value = "0";
                    if (value == "true")
                        value = "1";
...
function save1(url, data) {
    if ($("#VersionScreen").val() == 2)
    {
        url = "/GridCommon/InsertUpdatePopupMasterDetailV2/" + module + "/" + id;
    }
    ...
    if ($("#isUpdate").val() == "True")
    {
        ...
        if (un == unn) {
            url += "&isUpdate=true&valuePKOLD=" + pk;
        }
        else
        {
            url += "?isUpdate=true&valuePKOLD=" + pk;
    ...
    if (result.Status == 0) {
        ...
        if ($.isArray(result)) {
            ...
            for (var index in result) {
                var msg = ASOFT.helper.getMessage(result[index].Message);
                if (result[index].Data && typeof result[index].Data == "object" && result[index].Data.length > 1) {
                    var s = msg;
                    for (var i in result[index].Data) {
                        var reg = new RegExp("\\{" + i + "\\}", "gm");
                        s = s.replace(reg, result[index].Data[i]);
                    }
                    msg = s;
                }
                else {
                    msg = kendo.format(msg, result[index].Data);
                }
                arrayError.push(msg);
            }
            ASOFT.form.displayWarning('#' + id, arrayError);
        }
        else {
...
function deleteItems(tagA, tbGridDelete) {
    ...
    if (posGrid.dataSource.data().length > 1) {
        ...
        posGrid.dataSource.add(listAdd[tbGridDelete]);
    }
    return false;
}
