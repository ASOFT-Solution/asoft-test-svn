...
DRF2003 = new function () {
    ...
    this.SQL_Unique = null;
    this.isCommon = false;
    this.SQL_InsertImport = null;
    this.clUnique = null;
    ...
    */
    ...
    this.btnSave_ClickDynamic = function () {
        DRF2003.isReadData = false;
        DRF2003.displayBlock("progressBar", true);
        DRF2003.completed = false;
        DRF2003.page = 1;
        DRF2003.totalPage =
    ...
    this.savePaginate = function (errorCacheFile, transaction, completed, page, totalPage, idScreen, area, table, SQL_InsertImport, SQL_Unique, isCommon, clUnique) {
        ...
        if (postUrl) {
            ASOFT.helper.post(postUrl, {
                ...
                idScreen: idScreen,
                table: table,
                SQL_InsertImport: SQL_InsertImport,
                SQL_Unique: SQL_Unique,
                isCommon: isCommon,
                clUnique: clUnique
            }, function (data) {
    ...
    this.onUpload = function (data) {
        ...
        if (!DRF2003.type && $("#isDynamic").val() == "False") {
    ...
    this.onUploadSuccess = function (data) {
        if (data && data.response.counter > 0) {
            ...
            $(".k-upload-status-total").text(data.response.language);
        }
    };
    ...
    this.btnReadDataDynamic_Click = function () {
        DRF2003.isReadData = true;
        var postUrl = $("#UrlGetImportHeader").val();
        if (postUrl) {
            ASOFT.helper.post(postUrl, {
                filename: DRF2003.fileName,
                type: null,
                startRow: parseInt($("#StartRow").val()),
                area: $("#Area").val(),
                idScreen: $("#ScreenID").val(),
                table: $("#Table").val()
            }, DRF2003.appendImportHeaderGrid);
        }
    }
    ...
    this.appendImportHeaderGrid = function (data) {
        if (data) {
            if (!data.isTruthTemplete) {
                ...
                DRF2003.LanguagePage = data.language;
                ...
                DRF2003.SQL_Unique = data.SQL_Unique;
                DRF2003.SQL_InsertImport = data.SQL_InsertImport;
                DRF2003.isCommon = data.isCommon;
                DRF2003.clUnique = data.clUnique;
    ...
    this.bindingGridPaginate = function (dataRows, columns) {
        ...
        $('#imported-destination-table').kendoGrid({
            ...
            pageable: {
                ...
                messages: {
                    itemsPerPage: DRF2003.LanguagePage
                }
            },
            scrollable: true,
            selectable: "row",
        ...
        if (!isFine) {
            ...
            for (var i = 0; i < data.length; i++) {
                if (data[i].IsValidStatus && data[i].IsValidStatus == 1) {
                    ...
                    if ($("#isDynamic").val() == "True" && !DRF2003.isReadData)
                    {
                        var errorCols = DRF2003.splitMessage(data[i].ErrorColumns);
                        var strError = "";
                        for(var j = 0; j < errorCols.length - 1; j ++)
                        {
                            strError = strError + data[i][errorCols[j]] + ",";
                        }
                        strError = strError + data[i][errorCols[errorCols.length - 1]];

                        mgs = kendo.format(ASOFT.helper.getMessage(data[i].MessageForStatus), strError);
                    }
                    if (forceChangeMessage && ($("#isDynamic").val() == "False" || DRF2003.isReadData)) {
                        var args = [];
                        var para = data[i].ErrorColumn.split(',');
                        for (var j = 0; j < para.length; j++) {
                            args.push(data[i][para[j]]);
                        }

                        mgs = DRF2003.changeMessage(mgs, args);
                    }
                    if (typeof mgs !== "string" && mgs.length > 1) {
                        for (var j1 = 0; j1 < mgs.length; j1++) {
                            message.push(mgs[j1].toString());
                        }
                    } else {
                        message.push(mgs);
                    }
    ...
    this.changeMessage = function (messages, args) {
        var mgsChanged = [];
        for (var i = 0; i < messages.length; i++) {
            var mgs = messages[i].split('-');
            if (mgs[1] == 'OOFML000049') {                
                mgsChanged.push(mgs[0] + '-' + mgs[1].replace(mgs[1], kendo.format(ASOFT.helper.getMessage(mgs[1]), mgs[2], mgs[3])));
            } else {
                mgsChanged.push(messages[i].replace(mgs[1], kendo.format(ASOFT.helper.getMessage(mgs[1]), args[i])));
            }
        }
    ...
    this.bindingGridRow = function (dataSource) {
        ...
        $('#imported-destination-table').find(".k-pager-info").hide();
    };
    ...
    this.appendImportRowGrid = function (data) {
        ...
        //} else {
        //    ASOFT.form.displayMessageBox('#import-message', [ASOFT.helper.getMessage('00ML000087')], null);
    ...
    this.btnDownload_Click = function () {
        ...
        window.location = url + '?defaultFileName=' + dataItem.DefaultFileName.split(".")[0];
    }

    this.btnDownloadDynamic_Click = function () {
        defaultFileName = $("#Table").val();

        var url = $('#UrlDownload').val();

        window.location = url + '?defaultFileName=' + defaultFileName;
    }
}