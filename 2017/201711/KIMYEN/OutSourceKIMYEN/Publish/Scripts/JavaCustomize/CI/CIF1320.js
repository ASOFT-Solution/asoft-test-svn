var GridCIF1320 = null;
var urldeleteCIF1320 = null;

$(document).ready(function () {
    urldeleteCIF1320 = $('#DeleteCIF1320').val();
    GridCIF1320 = $("#GridAT1007").data("kendoGrid");
    $("#BtnDelete").unbind();
...
function CustomDelete_Click() {
    ...
    var records = ASOFT.asoftGrid.selectedRecords(GridCIF1320);
    if (records.length == 0) return;
    for (var i = 0; i < records.length; i++) {
        var valuepk = records[i]["VoucherTypeID"];
        args.push(valuepk);
    }
    key.push(tablecontent, pk);
    ASOFT.dialog.confirmDialog(ASOFT.helper.getMessage('00ML000024'), function () {
        ASOFT.helper.postTypeJson1(urldeleteCIF1320, key, args, deleteSuccess);
    });
}
...
function PrintClick() {
    ...
    ASOFT.helper.postTypeJson1("/CI/CIF1320/DoPrintOrExport", key1, value1, ExportSuccess);
}

function ExportSuccess(result) {
    if (result) {
        var urlPrint = '/CI/CIF1320/ReportViewer';
