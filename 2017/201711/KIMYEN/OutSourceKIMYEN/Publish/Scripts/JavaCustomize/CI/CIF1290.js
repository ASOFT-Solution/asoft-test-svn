var GridCIF1290 = null;
var urldeleteCIF1290 = null;

$(document).ready(function () {
    urldeleteCIF1290 = $('#DeleteCIF1290').val();
    GridCIF1290 = $("#GridAT1304").data("kendoGrid");
    $("#BtnDelete").unbind();
...
function CustomDelete_Click() {
    ...
    var records = ASOFT.asoftGrid.selectedRecords(GridCIF1290);
    if (records.length == 0) return;
    for (var i = 0; i < records.length; i++) {
        var valuepk = records[i]["UnitID"] + ",";
    ...
    ASOFT.dialog.confirmDialog(ASOFT.helper.getMessage('00ML000024'), function () {
        ASOFT.helper.postTypeJson1(urldeleteCIF1290, key, args, deleteSuccess);
    });
}
...
function PrintClick() {
    ...
    ASOFT.helper.postTypeJson1("/CI/CIF1290/DoPrintOrExport", key1, value1, ExportSuccess);
}

function ExportSuccess(result) {
    if (result) {
        var urlPrint = '/CI/CIF1290/ReportViewer';
