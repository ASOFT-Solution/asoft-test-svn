var GridCIF1090 = null;
var urldeleteCIF1090 = null;


$(document).ready(function () {
    urldeleteCIF1090 = $('#DeleteCIF1090').val();
    GridCIF1090 = $("#GridAT1207").data("kendoGrid");
    $("#BtnDelete").unbind();
...
function CustomDelete_Click() {
    ...
    var records = ASOFT.asoftGrid.selectedRecords(GridCIF1090);
    if (records.length == 0) return;
    for (var i = 0; i < records.length; i++) {
        var valuepk = records[i]["S"] + ",";
        var diviS = records[i]["DivisionID"] == "" ? "@@@" : records[i]["DivisionID"];
        valuepk = valuepk + diviS + "," + records[i]["STypeID"] + "," + records[i]["TypeID"];

        args.push(valuepk);
    }
    key.push(tablecontent, pk);
    ASOFT.dialog.confirmDialog(ASOFT.helper.getMessage('00ML000024'), function () {
        ASOFT.helper.postTypeJson1(urldeleteCIF1090, key, args, deleteSuccess);
    });
}
...
function PrintClick() {
    ...
    ASOFT.helper.postTypeJson1("/CI/CIF1090/DoPrintOrExport", key1, value1, ExportSuccess);
}

function ExportSuccess(result) {
    if (result) {
        var urlPrint = '/CI/CIF1090/ReportViewer';
