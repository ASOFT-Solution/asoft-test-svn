var GridCIF1300 = null;
var urldeleteCIF1300 = null;

$(document).ready(function () {
    urldeleteCIF1300 = $('#DeleteCIF1300').val();
    GridCIF1300 = $("#GridAT1313").data("kendoGrid");
    $("#BtnDelete").unbind();
...
function CustomDelete_Click() {
    ...
    var records = ASOFT.asoftGrid.selectedRecords(GridCIF1300);
    if (records.length == 0) return;
    for (var i = 0; i < records.length; i++) {
        var valuepk = records[i]["NormID"] + ",";
    ...
    ASOFT.dialog.confirmDialog(ASOFT.helper.getMessage('00ML000024'), function () {
        ASOFT.helper.postTypeJson1(urldeleteCIF1300, key, args, deleteSuccess);
    });
}
...
function PrintClick() {
    ...
    ASOFT.helper.postTypeJson1("/CI/CIF1300/DoPrintOrExport", key1, value1, ExportSuccess);
}

function ExportSuccess(result) {
    if (result) {
        var urlPrint = '/CI/CIF1300/ReportViewer';
