...
$(document).ready(function () {
    ...
    $("#tblFilter tbody").append($(".ResourceID"));

    $("#OORFilter").prepend(kendo.format(DivTag2block, 'divStatus'));
    $("#OORFilter").prepend(kendo.format(DivTag2block, 'divDepartAndDuty'));
    ...
    $("#divDepartAndDuty .block-left .asf-filter-label").append($(".DepartmentID_Traning").children()[0]);
    $("#divDepartAndDuty .block-left .asf-filter-input").append($($(".DepartmentID_Traning").children()[0]).children());
    $("#divDepartAndDuty .block-right .asf-filter-label").append($(".DutyID").children()[0]);
    $("#divDepartAndDuty .block-right .asf-filter-input").append($($(".DutyID").children()[0]).children());

    $("#divStatus .block-left .asf-filter-label").append($(".RecruitStatus").children()[0]);
    $("#divStatus .block-left .asf-filter-input").append($($(".RecruitStatus").children()[0]).children());
    ...
    dropdownList = $("#RecruitStatus").data('kendoDropDownList');
    if (dropdownList) {
        dropdownList.trigger("open");
        $($(dropdownList.list.find('li')[0])).trigger("click");
    }
    ...
    $(".DepartmentID_Traning").css('display', 'none');
    $(".DutyID").css('display', 'none');
    $(".RecruitStatus").css('display', 'none');

    $("#btnFromRecruitPeriodName").click(function () {
        var urlChoose = "/PopupSelectData/Index/HRM/HRMF2034?ScreenID=HRMF3010";
