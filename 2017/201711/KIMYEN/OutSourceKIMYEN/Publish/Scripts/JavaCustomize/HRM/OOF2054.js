...
$(document).ready(function () {
    ...
    GridOOT2030 = $("#GridEditOOT2030").data("kendoGrid");   
    GridOOT2030.hideColumn("APK");
    if ($("#IsLock").val() == "1") {
        $("#BtnSave").kendoButton({
            enable: false
        });
    ...
    if ($("#StatusOOT9001").val() != 1) {
        setTimeout(function () {
            var input = $("#GridEditOOT2030").find("input[type='checkbox']");
            $(input).each(function () {
                $(this).attr("onclick", "return false;");
            })
        }, 200)
    }
    GridOOT2030.unbind("dataBound");
    GridOOT2030.bind("dataBound", RowGrid_ChangeColor);
    var columns = GridOOT2030.columns;
    var name = 'GridEditOOT2030';
    $(GridOOT2030.tbody).off("keydown mouseleave", "td").on("keydown mouseleave", "td", function (e) {
        ...
        var isDefaultLR = $(GridOOT2030.element).attr('isDefaultLR');
        if (editor != undefined) {
            var elm = $(this);
            if (e.shiftKey) {
                switch (e.keyCode) {
...
function StatusChange() {
    GridOOT2030 = $('#GridEditOOT2030').data('kendoGrid');
    var data = GridOOT2030.dataSource.data();
    var totalNumber = data.length;
    if ($("#StatusOOT9001").val() == 1) {
        var input = $("#GridEditOOT2030").find("input#CbGridEdit_Status[type='checkbox']");
        $(input).each(function () {
...
function CustomerCheck() {
    ...
    if ($("#StatusOOT9001").val() == 0) {
        ...
        ASOFT.form.displayError("#OOF2054", msg);
        isChecked = true;
    }
    if (!isChecked) {
