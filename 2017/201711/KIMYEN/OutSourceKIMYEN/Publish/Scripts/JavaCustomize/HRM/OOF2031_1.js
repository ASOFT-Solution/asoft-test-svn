...
$(document)
    .ready(function() {
                ...
                function(e) {
                    ...
                    if (column == 'WorkFromDate') {
                        ...
                        if (selectitem.WorkFromDate && selectitem.WorkToDate) {
                            var diff = Date.parse(changeDate(selectitem.WorkToDate)) -
                                Date.parse(changeDate(selectitem.WorkFromDate));
        ...
        if ($('#isInherit').val() == 'True') {
            ...
            $(grid.tbody)
                .off("keydown mouseleave", "td")
                .on("keydown mouseleave",
                    "td",
                    function(e) {
        ...
        GRID_AUTOCOMPLETE.config({
            ...
            setDataItem: function(selectedRowItem, dataItem) {
                ...
                ASOFT.helper.postTypeJson("/HRM/OOF2030/CheckDataOvertime40",
                    data1,
                    function(result) {
                        if (result.isCheck == 1) {
                            ...
                            ASOFT.form.displayMessageBox('#' + $('#sysScreenID').val(),
                                [ASOFT.helper.getMessage('OOFML000036').f(dataSource[i].EmployeeID)],
                                null);
                        }
                    });
