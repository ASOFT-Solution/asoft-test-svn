...
$(document).ready(function () {
    OOF2040.grid = $('#GridOOT9000').data('kendoGrid');

    var cboDepartment = $("#DepartmentID_OOF2040").data("kendoComboBox");
    cboDepartment.bind("change", function (e) {
        $("#SectionID_OOF2040").data("kendoComboBox").text('');
        $("#SubsectionID_OOF2040").data("kendoComboBox").text('');
        $("#ProcessID_OOF2040").data("kendoComboBox").text('');

        cboDepartment.input.focus();
    });

    var cboSection = $("#SectionID_OOF2040").data("kendoComboBox");
    ...
    var cboSubsection = $("#SubsectionID_OOF2040").data("kendoComboBox");
...
function PrintClick() {
    ...
    OOF2040.doPrintOrExport(true);
   
}

function onAfterFilter() {
    OOF2040.isSearch = true;
    var datamaster = ASOFT.helper.dataFormToJSON("FormFilter");
    sessionStorage.setItem('dataFormFilter_OOF2040', JSON.stringify(datamaster));
}

function onAfterClearFilter() {
    OOF2040.isSearch = false;
...
function onRefreshGrid() {
    var totalPage = OOF2040.grid.pager.options.buttonCount;
    var CurPage = OOF2040.grid.pager.page();
    if (IsCheckExecute) {
        OOF2040.grid.dataSource.page(CurPage <= totalPage ? CurPage : 1);
        IsCheckExecute = false;
    } else {
        OOF2040.grid.dataSource.page(1);
    }
}
...
OOF2040 = new function () {
    ...
    this.doPrintOrExport = function (ischecked) {
        ...
        var dataFormFilter = sessionStorage.getItem('dataFormFilter_OOF2040');
        ...
        data.IsSearch = OOF2040.isSearch;
        ...
        data.ReportID = 'OOF2084Report';
        isPrint = ischecked;

        data.APKList = args.join(",");
        var urlPost = "/HRM/OOF2040/DoPrintOrExport";
        ASOFT.helper.postTypeJson(urlPost, data, OOF2040.exportOrPrintSuccess);
    }

    this.exportOrPrintSuccess = function (result) {
        if (result) {
            if (result.isDataSource) {
                var urlPrint = '/HRM/OOF2040/ReportViewer';
                var urlExcel = '/HRM/OOF2040/GetReportFile';
