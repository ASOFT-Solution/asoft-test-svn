            ...
            function(key, value) {
                ...
                ASOFT.helper.post(url,
                    ...
                    function(data) {
                        if (data.length > 0) {
                            for (var i = 0; i < data.length; i++) {
                                message.push(ASOFT.helper.getMessage("OOFML000029")
                                    .f(data[i].EmployeeID, data[i].Date.split(' ')[0], data[i].ShiftID));
                            }
                            ASOFT.form.displayMessageBox('#' + $('#sysScreenID').val(), message, null);
                            check = true;
                        }
        ...
        $.each(data,
            function (key, value) {
                sendData.push({
                    ...
                    LeaveFromDate: kendo.toString(kendo.parseDate(value.ChangeFromDate, "dd/MM/yyyy"), "s").split('T').join(' '),
                    LeaveToDate: kendo.toString(kendo.parseDate(value.ChangeToDate, "dd/MM/yyyy"), "s").split('T').join(' ')
                });
            });
        ...
        for (var i = 0; i < rowList.length; i++) {
            for (var j = 0; j < listCheck.length; j++) {
                ...
                if (row.children()[columnIndexEmployeeID].textContent.trim() == listCheck[j].EmployeeID &&
                    (row.children()[columnIndexChangeFromDate].textContent) == format_date(new Date(listCheck[j].LeaveFromDate)) &&
                    (row.children()[columnIndexChangeToDate].textContent) == format_date(new Date(listCheck[j].LeaveToDate))) {
