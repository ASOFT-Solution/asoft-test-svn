...
$(document).ready(function () {
    ...
    $("#popupInnerIframe").kendoWindow({
        activate: function () {
            ...
            WMF3003.cboFromObject = $("#" + WMF3003.FROMOBJECTID).data("kendoComboBox");
            $("#" + WMF3003.FROMOBJECTID).on('change', fromobject_cbo_onchange);

            WMF3003.cboToObject = $("#" + WMF3003.TOOBJECTID).data("kendoComboBox");
            $("#" + WMF3003.TOOBJECTID).on('change', toobject_cbo_onchange);
...
function fromobject_cbo_onchange() {
    var selected_item = WMF3003.cboFromObject.dataItem(WMF3003.cboFromObject.select());
    if (selected_item) {
...
function toobject_cbo_onchange() {
    var selected_item = WMF3003.cboToObject.dataItem(WMF3003.cboToObject.select());
    if (selected_item) {
        var ObjectName = selected_item.ObjectName;
...
function AppendSubTextBox() {
    var fromobject_current = $("#" + WMF3003.FROMOBJECTID).parent();
    var toobject_current = $("#" + WMF3003.TOOBJECTID).parent();
    var warehouse_parent = $("#" + WMF3003.WAREHOUSEID).parent();
    var frominventory_parent = $("#" + WMF3003.FROMINVENTORYID).parent();
    var toinventory_parent = $("#" + WMF3003.TOINVENTORYID).parent();
...
function CustomerCheck() {
    ...
    if (WMF3003.GetFromObjectID() == "") {
        null_fromobject = true;
        WMF3003.AddFromObjectIDError();
    }
    else {
        WMF3003.ClearFromObjectIDError();
    }

    if (WMF3003.GetToObjectID() == "") {
        null_toobject = true;
        WMF3003.AddToObjectIDError();
    }
    else {
        WMF3003.ClearToObjectIDError();
    }
