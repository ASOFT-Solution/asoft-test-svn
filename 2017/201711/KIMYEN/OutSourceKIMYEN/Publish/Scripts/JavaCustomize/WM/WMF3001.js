...
$(document).ready(function () {
    ...
    $("#popupInnerIframe").kendoWindow({
        activate: function () {
            WMF3001.cboObjectID = $("#" + WMF3001.OBJECTID).data("kendoComboBox");
            $("#" + WMF3001.OBJECTID).on('change', object_cbo_onchange);
...
function AppendSubTextBox() {
    var object_current = $("#" + WMF3001.OBJECTID).parent();
...
WMF3001 = new function () {
    ...
    this.cboObjectID = null;
    // Setting Init
    this.SettingInit = function () {
        // Xử lý combo đối tượng
        var object_data = this.cboObjectID.dataSource._data;
        if (object_data.length > 0) {
            this.cboObjectID.select(0);
            var selected_item = this.cboObjectID.dataItem(this.cboObjectID.select());
            if (selected_item) {
                $("#" + txtObjectName).val(selected_item.ObjectName);
            }
        }
    ...
    this.GetObjectID = function () {
        var selected_item = this.cboObjectID.dataItem(this.cboObjectID.select());
        if (selected_item) {
            return selected_item.ObjectID;
        }
        return "";
    }

    this.AddObjectIDError = function () {
        var Object = $("#" + this.OBJECTID);
        if (!Object.parent().hasClass('asf-focus-input-error')) {
            Object.parent().addClass('asf-focus-input-error');
        }
    }

    this.ClearObjectIDError = function () {
        var Object = $("#" + this.OBJECTID);
        if (Object) {
            if (Object.parent().hasClass('asf-focus-input-error')) {
                Object.parent().removeClass('asf-focus-input-error');
            }
        }
    }
}
...
function CustomerCheck() {
    ...
    if (WMF3001.GetObjectID() == "") {
        no_objectdetails = true;
        WMF3001.AddObjectIDError();
    }
    else {
        WMF3001.ClearObjectIDError();
    }
...
function object_cbo_onchange() {
    var selected_item = WMF3001.cboObjectID.dataItem(WMF3001.cboObjectID.select());
    if (selected_item) {
        var ObjectName = selected_item.ObjectName;
        $("#" + txtObjectName).val(ObjectName);
    }
    else {
        $("#" + txtObjectName).val(ObjectName);
    }
