...
$(document).ready(function () {
    ...
    $("#popupInnerIframe").kendoWindow({
        activate: function () {
            $.ajax({
                url: '/Partial/PartialFilterWMF20037',
                type: "GET",
                async: false,
                success: function (result) {
                    $("#WMF2007").prepend(result);
            ...
            if (isStatus == "Update") {
                ...
                for (var i = 0; i < grid_update_data.length; i++) {
                    ...
                    WMF2007.UpdateInventoryList(inventory);
                    if (grid_update_data[i].ReVoucherNo && grid_update_data[i].ReVoucherNo.length > 0) {
    ...
    $(grid.tbody).on("change", "td", function (e) {
        ...
        if (column == 'cbbReVoucherNo' && !null_value) {
            ...
            var dataSource = $('#cbbReVoucherNo').data('kendoComboBox').dataItem();
            if (dataSource) {
                selectitem.SourceNo = dataSource.ReSourceNo;
                selectitem.UnitPrice = dataSource.UnitPrice;
                selectitem.ActualQuantity = dataSource.EndQuantity;
                selectitem.ReVoucherID = dataSource.ReVoucherID;
                selectitem.ReTransactionID = dataSource.ReTransactionID;
                if (selectitem.ActualQuantity && selectitem.UnitPrice)
                ...
                selectitem.LimitDate = dataSource.LimitDate.split(' ')[0];
            }
            grid.refresh();
        }
    });
    ...
    $(grid.tbody).on("focusout", "td", function (e) {
        ...
        if (column == 'cbbReVoucherNo' || e.target.name.split('_')[0] == 'ReVoucherNo') {
            ...
            if (selectitem.InventoryID == "" || selectitem.ReVoucherNo == "" || selectitem.ReVoucherNo.length <= 0) {
                ...
                var cbboReVoucherNo = $('#cbbReVoucherNo').data('kendoComboBox');
                if (cbboReVoucherNo) {
                    var dataSource = cbboReVoucherNo.dataItem();
                    if (dataSource) {
                        selectitem.SourceNo = dataSource.ReSourceNo;
                        selectitem.ReVoucherID = dataSource.ReVoucherID;
                        selectitem.ReTransactionID = dataSource.ReTransactionID;
                ...
                if (data[i].ActualQuantity < 0) {
                    ...
                    if (isStatus == "AddNew") {
                        ...
                        var dataPost = {
                            ...
                            ReOldVoucherID: "",
                            ReOldTransactionID: "",
                            ReNewVoucherID: data[i].ReVoucherID,
                            ReNewTransactionID: data[i].ReTransactionID,
