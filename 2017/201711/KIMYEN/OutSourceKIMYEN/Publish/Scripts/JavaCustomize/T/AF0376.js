$(document).ready(function () {
    ...
    $("#TableOO1").append(kendo.format(DivTag, 'DPeriod'));
    ...
    $('#DPeriod .block-left .asf-filter-label').append($(".Period").children()[0]);
    $('#DPeriod .block-left .asf-filter-input').append($(".Period").children()[0]);
    ...
    var cboPeriod = $("#Period").data("kendoComboBox");
    if (cboPeriod) {
        cboPeriod.select(0);
        cboPeriod.bind('change', function () {
            var data = cboPeriod.dataItem();
            $("#Month").val(data.TranMonth);
            $("#Year").val(data.TranYear);
        });
        var data = cboPeriod.dataItem();
        $("#Month").val(data.TranMonth);
        $("#Year").val(data.TranYear);
