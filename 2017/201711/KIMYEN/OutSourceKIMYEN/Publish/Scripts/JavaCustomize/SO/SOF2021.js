...
var isConfirm = false;
var btnFromObjectID = null;
var btnDeleteObjectID = null;
...
$(document).ready(function () {
    ...
    btnFromObjectID = '<a id="btnFromObjectID" style="z-index:10001; position: absolute; right: 28px; height: 25px ; min-width: 27px;" data-role="button" class="k-button k-button-icontext asf-button" role="button" aria-disabled="false" tabindex="0" onclick="btnChooseObjectID_Click()">...</a>';

    btnDeleteObjectID = '<a id="btnDeleteObjectID" style="z-index:10001; position: absolute; right: 0px; height: 25px; min-width: 27px; border: 1px solid #dddddd;" data-role="button" class="k-button k-button-icontext  asf-i-delete-32" role="button" aria-disabled="false" tabindex="0" onclick="btnDeleteObjectID_Click(1)"></a>';

    var btnOpportunityID = '<a id="btnOpportunityID" style="z-index:10001; position: absolute; right: 28px; height: 25px ; min-width: 27px;" data-role="button" class="k-button k-button-icontext asf-button" role="button" aria-disabled="false" tabindex="0" onclick="btnOpportunityID_Click()">...</a>';

    var btnDeleteOpportunityID = '<a id="btnDeleteOpportunityID" style="z-index:10001; position: absolute; right: 0px; height: 25px; min-width: 27px; border: 1px solid #dddddd;" data-role="button" class="k-button k-button-icontext  asf-i-delete-32" role="button" aria-disabled="false" tabindex="0" onclick="btnDeleteOpportunityID_Click(1)"></a>';
    ...
    $(".ObjectName").before($(".OpportunityName"));
    $("#OpportunityName").after(btnOpportunityID);
    $("#OpportunityName").after(btnDeleteOpportunityID);
    $("#OpportunityName").attr('disabled', 'disabled');
    ...
    GridOT2102.bind('dataBound', function (e) {
        $(GridOT2102.tbody).find('td').on("keyup", function (e) {
            if (e.keyCode == 119) {
                var tr = e.currentTarget.parentElement;
                var dataItem = GridOT2102.dataItem(tr);

                if (e.target.id === "OriginalAmount") {
                    if (e.target != null && dataItem.QuoQuantity != null) {
                        var unit = e.target.value / dataItem.QuoQuantity;
                        dataItem.set("UnitPrice", unit);
                    }
                }
                else {
                    if (dataItem.OriginalAmount != null && dataItem.QuoQuantity != null) {
                        var unit = dataItem.OriginalAmount / dataItem.QuoQuantity;
                        dataItem.set("UnitPrice", unit);
                    }
                }
            }
    ...
    $(GridOT2102.tbody).on("click", "td", function (e) {
        var id = e.target.id;
    })
...
function btnOpportunityID_Click() {
    urlChooseAccount = "/PopupSelectData/Index/CRM/CRMF9013?DivisionID=" + $("#DivisionID").val();
    ASOFT.form.clearMessageBox();
    ASOFT.asoftPopup.showIframe(urlChooseAccount, {});
    action = 5;
}

function btnDeleteOpportunityID_Click() {
    if ($("#OpportunityID").val() != "") {
        $("#APKOpportunityID").val("");
        $("#OpportunityID").val("");
        $("#OpportunityName").val("");
        $("#ObjectID").val("");
        $("#ObjectName").val("");
...
function receiveResult(result) {
    ...
    if (action == 5) {
        $("#OpportunityID").val(result["OpportunityID"]);
        $("#OpportunityName").val(result["OpportunityName"]);
        $("#APKOpportunityID").val(result["APK"]);

        ASOFT.helper.postTypeJson("/SO/SOF2020/LoadObjectID", { OpportunityID: result["OpportunityID"] }, function (rsData) {
            if (rsData) {
                $("#ObjectID").val(rsData["AccountID"]);
                $("#ObjectName").val(rsData["AccountName"]);
                $("#Tel").val(rsData["Tel"]);
                $("#Address").val(rsData["Address"]);
                $("#DeliveryAddress").val(rsData["DeliveryAddress"]);
                if ($("#ObjectID").val() != "" && $("#ObjectName").val() != "") {
                    $("a#btnFromObjectID").remove();
                    $("a#btnDeleteObjectID").remove();
                }
                else {
                    if ($("a#btnFromObjectID").length == 0) {
                        $("#ObjectName").after(btnFromObjectID);
                        $("#ObjectName").after(btnDeleteObjectID);
                    }
                }
