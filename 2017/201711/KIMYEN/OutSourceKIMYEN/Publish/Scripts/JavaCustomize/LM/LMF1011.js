...
$(document).ready(function () {
    ...
    $(grid.tbody).on("change", "td", function (e) {
        ...
        if (column == 'cbbCreditFormID') {
            ...
            selectitem.CreditTypeName = value.CreditTypeName;
            grid.refresh();
        }
    });
    ...
    $('#OriginalLimitTotal').attr('onchange', 'OriginalLimitTotal_Change()')

   

    formatForNumericTextBox("OriginalLimitTotal", ASOFTEnvironment.NumberFormat.KendoExchangeRateDecimalsFormatString, null, null, { isDefault: true, Value: 0 }, k_NumericTextBox_ConvertAmount_Change, ASOFTEnvironment.NumberFormat.ExchangeRateDecimals);

    formatForNumericTextBox("ConvertedLimitTotal", ASOFTEnvironment.NumberFormat.KendoConvertedDecimalsFormatString, null, null, { isDefault: true, Value: 0 }, k_NumericTextBox_ConvertAmount_Change, ASOFTEnvironment.NumberFormat.ConvertedDecimals);
})

function OriginalLimitTotal_Change() {
    var dataExchangeRate = $('#ExchangeRate').data("kendoNumericTextBox");
    var dataOriginalLimitTotal = $('#OriginalLimitTotal').data("kendoNumericTextBox");
    var dataConvertedLimitTotal = $('#ConvertedLimitTotal').data("kendoNumericTextBox");
    var totalConvertedAmount = parseFloat("0");
    if (pOperator == '0') {
        totalConvertedAmount = dataOriginalLimitTotal.value() * dataExchangeRate.value();
    } else {
        totalConvertedAmount = dataOriginalLimitTotal.value() / dataExchangeRate.value();
    }
    dataConvertedLimitTotal.value(totalConvertedAmount);
}
...
function ExchangeRate_Change() {
    ...
    var totalOriginalLimit = parseFloat('0');
    var totalConvertedLimit = parseFloat('0');
    for (var i = 0; i < originalLimitAmount.length; i++) {
        totalOriginalLimit = totalOriginalLimit + originalLimitAmount[i];
        if (pOperator == 0) {
            dataSource[i].ConvertedLimitAmount = originalLimitAmount[i] * dataExchangeRate.value();
        } else {
            dataSource[i].ConvertedLimitAmount = originalLimitAmount[i] / dataExchangeRate.value();
        }
        totalConvertedLimit = totalConvertedLimit + dataSource[i].ConvertedLimitAmount;
    }
    ...
    function cboCurrencyID_Change(e) {
        ...
        if (dataItem) {
            pOperator = parseInt(dataItem.Operator ? dataItem.Operator : '0');
    ...
    function Grid_SaveCustom(e) {
        ...
        if (e.values != null)
        {
            if (e.values.hasOwnProperty("OriginalLimitAmount") && (e.values.OriginalLimitAmount || e.values.OriginalLimitAmount == null)) {
                var pOriginalLimitAmount = parseFloat(e.values.OriginalLimitAmount ? e.values.OriginalLimitAmount : 0);
                var pConvertedLimitAmount = 0;
                var dataExchangeRate = $('#ExchangeRate').data("kendoNumericTextBox");
                if (pOperator == '0') {
                    pConvertedLimitAmount = pOriginalLimitAmount * dataExchangeRate.value();
                } else {
                    pConvertedLimitAmount = pOriginalLimitAmount / dataExchangeRate.value();
                }
            ...
            if (e.values.hasOwnProperty("ConvertedLimitAmount") && (e.values.ConvertedLimitAmount || e.values.ConvertedLimitAmount == null)) {
                e.model.set("ConvertedLimitAmount", e.values.ConvertedLimitAmount ? e.values.ConvertedLimitAmount : 0);
                getDataMaster();
            }
    ...
    function getDataMaster() {
        ...
        if (dataSource) {
            $(dataSource).each(function () {
                totalOriginalLimit = parseFloat(totalOriginalLimit) + parseFloat(this.OriginalLimitAmount ? this.OriginalLimitAmount.toString().replace(/,/g, '') : '0');
                totalConvertedLimit = parseFloat(totalConvertedLimit) + parseFloat(this.ConvertedLimitAmount ? this.ConvertedLimitAmount.toString().replace(/,/g,'') : '0');
            });
        }
        var dataOriginalLimitTotal = $('#OriginalLimitTotal').data("kendoNumericTextBox");
        var dataConvertedLimitTotal = $('#ConvertedLimitTotal').data("kendoNumericTextBox");
    ...
    function CustomerCheck() {
        ...
        if (!check) {
            if (parseFloat(totaloriginalLimitAmount) != parseFloat($('#OriginalLimitTotal').val().replace(/,/g, ''))) {
            ...
            if (parseFloat(totalconvertedLimitAmount) != parseFloat($('#ConvertedLimitTotal').val().replace(/,/g, ''))) {
